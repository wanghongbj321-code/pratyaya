...
- **禁止新增或使用渲染脚本**（如 `scripts/render_canvas.py` 等任何把数据写成 HTML 的脚本）；渲染统一通过调用 `canvas-render` Skill 完成，参照 `examples/canvas-html/` 对应画布示例模板生成画布。
- `scripts/audit_canvas_html.py` 是**审计**工具（校验产物结构与授权元数据），不是渲染工具；渲染后必须用它做静态审计。
