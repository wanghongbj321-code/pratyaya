...
- 五类画布（MVL / 黄金圈 / HMW / 用户画像 / 用户旅程）的引导、转写提炼与确认包沉淀；非 MVL 一等公民画布支持同一 group 多 instance
- 质量门禁（Gate）建议 + 用户授权
- 模块化智能体画布（Canvas）的生成（HMW / Persona / Journey 走双 Gate 审计）
- 5 态画布生命周期管理（五类画布共用）
...
> `confirmation_mode` 是属性（不是状态）；用户可对 `business_risk` 显式 override 后进入 `confirmed`，并触发 caveat 显式呈现。HMW / Persona / Journey 正式渲染额外要求结构 Template Gate 通过（详见 [DEVELOPMENT.md §3.1](../DEVELOPMENT.md#31-python-静态审计)）。
...
- 检查 `skills/` 目录完整（应含 11 个 Skill：mvl-distill / gc-distill / hmw-distill / persona-distill / journey-distill / module-conclusion-gate / gc-gate / hmw-gate / persona-gate / journey-gate / canvas-render）
...
- 正式 Canvas 顶部应有画布名（MVL Canvas / Golden Circle / HMW Canvas / Persona Canvas / Journey Canvas）+ 版本号；非 MVL instance 页还应绑定 `data-instance`
- 如无区分标志，请重新安装
- HMW / Persona / Journey 正式 Canvas 无法生成时，检查是否已通过双 Gate（`--template` 缺失会 FAIL）；非 MVL instance 页还需检查审计命令是否携带 `--instance {slug}`
