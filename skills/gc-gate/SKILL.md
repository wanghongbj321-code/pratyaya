---
name: gc-gate
description: 对黄金圈（Golden Circle）工作坊的确认包（GC-{slug}-v{N}.md）做质量建议与风险分级，输出 gate_recommendation 供主 Agent 决策。Gate 只输出建议，不决定最终渲染授权；用户授权由主 Agent 根据 Gate 报告与用户决策写入。本 skill 不生成 HTML，也不替用户补结论。
---

# 黄金圈结论质量建议

把"内容已讨论过"与"内容足以成为黄金圈品牌定位结论"分开。本 skill 只做质量建议和风险分级，**不决定最终渲染授权**。最终授权由主 Agent 在用户决策后写入 `state.json` 的 `render_authorized` 与 `confirmation_mode` 字段。
...
- `modules/GC-{slug}-v{N}.md`（确认包，Markdown 格式，唯一事实源）
- `references/GC-gate.md`（黄金圈放行条件与稳定 ID，位于本 Skill 的 `references/` 子目录）
- 当前状态（来自主 agent 维护的 `state.json` 的 `golden_circle` 区块）
...
**触发**：主 agent GC 工作流对应步骤——确认包已生成、状态为 `review_ready`，主 Agent 自动调用本 skill。

1. 读取 `modules/GC-{slug}-v{N}.md`（确认包第 1–11 节业务内容；第 12 节为治理元数据，不参与评估）；
2. 输出 Gate 报告文件名使用 `modules/GC-{slug}-gate-report-v{N}.md`；slug 不进入 `GC-GATE-XX` 稳定 ID。
2. 读取 `references/GC-gate.md`（黄金圈放行条件 + 稳定 ID + 分类 + 风险等级）；
...
|---|---|---|---|---|---|---|---|
| GC-GATE-01 | WHY 三子字段覆盖 | PASS / FAIL | information_integrity / business_risk | low / medium / high | GC-why | ... | ... |
| GC-GATE-02 | HOW 三子字段覆盖 | ... | ... | ... | ... | ... | ... |
...
6 条放行条件在 `references/GC-gate.md` 中拥有稳定 ID：
...
| `business_risk` | 结论已有事实基础，但逻辑推导或现实验证仍不完整（跨层一致性、市场证据充分度等） | **是**（填写理由后） |
...
| 等级 | 影响 | 处置 |
|---|---|---|
| `blocker` | 使某层核心产出或对应 Canvas section 无法成立 | 必须关闭，不允许接受风险 |
| `major` | 会显著改变某层的判断或方向 | 必须关闭或接受风险 |
...
- 第 6a 节"跨层一致性"是否填写，WHY→HOW 和 HOW→WHAT 是否都有判断
- 一致性判断为"未建立"时，是否在缺口表中登记对应的 major 缺口
- MC-HOW 的差异化是否能在确认包中找到与 WHY 信念的逻辑关联（非 LLM 构造）

> **注意**：黄金圈是单画布，不存在跨模块 caveat 检查。跨层一致性检查只在本 Gate 内完成。

## 草稿与正式版

- 草稿 Canvas 仅用于继续讨论，必须带"草稿 / 未确认 / 禁止用于管理层决策"水印。
- 草稿不进入正式输出。
- 正式 Canvas 必须来自已确认的同一版本确认包，且对应 `state.json.golden_circle.{slug}.render_authorized=true`、`confirmation_mode ∈ {gate_pass, override}`；确认包 `{slug}`、state key、`state.json.golden_circle.{slug}.slug` 与输出文件名必须一致。
...
8. 确认人未填写���角色不清的不得建议 `gate_recommendation=pass`。
