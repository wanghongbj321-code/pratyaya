---
name: 5w-gate
description: 对丰田 5W（Five Whys，根因分析）工作坊的确认包（5W-{slug}-v{N}.md）做质量建议与风险分级，输出 gate_recommendation 供主 Agent 决策。Gate 只输出建议，不决定最终渲染授权；用户授权由主 Agent 根据 Gate 报告与用户决策写入。本 skill 不生成 HTML，也不替用户补结论。
---

# 5W 结论质量建议

把"内容已讨论过"与"内容足以成为 5W 根因分析结论"分开。本 skill 只做质量建议和风险分级，**不决定最终渲染授权**。最终授权由主 Agent 在用户决策后写入 `state.json` 的 `render_authorized` 与 `confirmation_mode` 字段。

## 定位

**质量建议器（Gate）**：本 skill 是 Pratyaya Coach 的"质量建议"能力。完整工作流由主 agent 编排（见 `agents/pratyaya.md`），本 skill 不编排主流程，只在被调用时输出 Gate 建议报告（Markdown）。
...
- `modules/5W-{slug}-v{N}.md`（确认包，Markdown 格式，唯一事实源）
- `references/5W-gate.md`（5W 放行条件与稳定 ID，位于本 Skill 的 `references/` 子目录）
- 当前状态（来自主 agent 维护的 `state.json` 的 `five_whys` 区块）
...
**触发**：主 agent 5W 工作流对应步骤——确认包已生成、状态为 `review_ready`，主 Agent 自动调用本 skill。

1. 读取 `modules/5W-{slug}-v{N}.md`（确认包第 1–16 节业务内容；第 17 节为治理元数据，不参与评估）；
2. 输出 Gate 报告文件名使用 `modules/5W-{slug}-gate-report-v{N}.md`；slug 不进入 `5W-GATE-XX` 稳定 ID。
2. 读取 `references/5W-gate.md`（5W 放行条件 + 稳定 ID + 分类 + 风险等级）；
...
|---|---|---|---|---|---|---|---|
| 5W-GATE-01 | 问题陈述是事实而非结论 | PASS / FAIL | information_integrity / business_risk | low / medium / high | 5W-problem | ... | ... |
| 5W-GATE-02 | Why 1-5 每层有内容或显式标为缺口 | ... | ... | ... | 5W-why-1..5 | ... | ... |
| 5W-GATE-03 | 每层"因为"答案附证据 | ... | ... | ... | 5W-why-1..5 | ... | ... |
| 5W-GATE-04 | 无个人归因 | ... | ... | ... | 5W-why-1..5, 5W-rubric | ... | ... |
| 5W-GATE-05 | 因果链通过"因此"检验 | ... | ... | ... | 5W-root | ... | ... |
| 5W-GATE-06 | 根因可行动且对策四要素齐全 | ... | ... | ... | 5W-root, 5W-countermeasures | ... | ... |
| 5W-GATE-07 | 对策是预防性回应而非症状修复 | ... | ... | ... | 5W-countermeasures | ... | ... |
...
7 条放行条件在 `references/5W-gate.md` 中拥有稳定 ID：
...
| 分类 | 含义 | 用户 override |
|---|---|---|
| `information_integrity` | 正式产物能否真实成立（事实陈述、证据、反归因等因果链诚实性） | **否** |
...
1. 问题陈述（第 6 节，事实表述）；
2. 因果链五层（第 7 节，含层面 / 追问对象 / 答案 / 证据 / 检查点）；
3. 根本原因与"因此"检验（第 8 节）；
4. 对策四要素（第 9 节）；
5. 判别记录（第 11 节）；
6. 结论及共识状态（第 12 节）；
...
每个模块在进入 `review_ready` 之前，必须完成对齐检查，并将结果写入确认包的对齐 section（第 2 节对齐摘要）。
...
- 第 6 节"问题陈述"是否为事实表述（非"团队混乱"类结论）；若由结论改写而来，是否标注改写
- 第 7 节"因果链"每层是否有内容或显式标为缺口，证据列是否填写（日期 / 记录 / 日志）
- 第 11 节"判别记录"是否只记录实际发生的打回对照（无打回记录时写"无"，不得编造示例）
- 第 8 节"根本原因"是否通过"因此"检验三连问（停止准则）

> **注意**：5W 是单画布，不存在跨模块 caveat 检查。因果链诚实性检查只在本 Gate 内完成。

## 草稿与正式版

- 草稿 Canvas 仅用于继续讨论，必须带"草稿 / 未确认 / 禁止用于管理层决策"水印。
- 草稿不进入正式输出。
- 正式 Canvas 必须来自已确认的同一版本确认包，且对应 `state.json.five_whys.{slug}.render_authorized=true`、`confirmation_mode ∈ {gate_pass, override}`；确认包 `{slug}`、state key、`state.json.five_whys.{slug}.slug` 与输出文件名必须一致。
