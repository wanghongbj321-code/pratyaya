---
name: hmw-distill
description: 把 HMW（How Might We，问题重构）工作坊讨论产物提炼为 Markdown 资产。先做 Key Points 概览抽取（生成陈述四字段 + 质量鉴别 + 想法种子的讨论地图），再按用户决策做原子提炼，输出唯一事实源 HMW-{slug}-v{N}.md 确认包。收到 HMW Key Points 抽取请求、原子提炼请求、确认包生成请求时使用。
---

# hmw-distill：HMW 问题重构提炼

把 Key Points 概览与 HMW 框架结合，形成经过对齐的、唯一事实源的确认包（`modules/HMW-{slug}-v{N}.md`）。完成标准是"经业务方、技术方、管理层各自动用都能成立的问题重构资产"，不是"看起来内容丰富"。

所有 HMW 产物必须绑定 instance slug。slug 由主 Agent 提供，必须为 kebab-case，并与 `state.hmw.{slug}.slug` 一致；本 Skill 不自动生成 `default` slug。

## 定位

**分步提炼流程**：本 skill 是 Pratyaya Coach 的"HMW 提炼与确认包生成"能力。完整的 HMW 工作流由主 agent 编排（见 `agents/pratyaya.md`），本 skill 不编排主流程，只在被调用时执行以下两个独立 Stage：

- **Stage 1：Key Points 抽取** — 输入转写，输出 `modules/HMW-{slug}-keypoints.md`（讨论地图，30 秒浏览）。
- **Stage 2：原子提炼** — 输入转写 + Key Points + HMW 框架，输出 `modules/HMW-{slug}-v{N}.md`（确认包，唯一事实源）。
...
| Stage | 输入 | 输出 |
|---|---|---|
| Stage 1：Key Points 抽取 | 逐字稿（文本或文件路径） | `modules/HMW-{slug}-keypoints.md`（第 N 轮） |
| Stage 2：原子提炼 | 逐字稿 + Key Points + HMW 框架 | `modules/HMW-{slug}-v{N}.md`（确认包，唯一事实源） |
...
**输出**：`modules/HMW-{slug}-keypoints.md`，结构如下。

### HMW-{slug}-keypoints.md 模板
...
**目标**：将 Key Points 转化为经过对齐的、唯一事实源的确认包 `HMW-{slug}-v{N}.md`。
...
- 逐字稿（已存档）
- `modules/HMW-{slug}-keypoints.md`（Stage 1 产物）
- `frameworks/hmw-frame.md`（HMW 框架）

**输出**：`modules/HMW-{slug}-v{N}.md`，全 Markdown，结构如下。

### HMW-{slug}-v{N}.md 确认包模板
...
- 版本号管理：升版时 vN → vN+1，旧版本归档为 `modules/HMW-{slug}-v{N}.md.previous`，不清空。
...
- **业务内容变化**：必须 `version + 1` + `gate_recommendation=pending` + `render_authorized=false` + `confirmation_mode=null` + 清空当前版本 `override_audit`；旧版本确认包归档为 `HMW-{slug}-v{N}.md.previous`，旧版第 12 节审计随旧版保留。
- **治理元数据写入**（仅第 12 节）：不触发升版；Gate 报告摘要与用户授权属于当前版本的元数据补充。
- **历史版本审计**不得清空：旧版 `HMW-{slug}-v{N}.md.previous` 的第 12 节（包括历史 override 审计）必须完整保留，用于追溯。

## 元数据生成时间字段

- `生成时间`（`HMW-{slug}-v{N}.md` 顶部）由本 skill 在 Stage 2 生成确认包时按系统真实时间写入（ISO 8601 datetime）。
