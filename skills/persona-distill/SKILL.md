...
**分步提炼流程**：本 skill 是 Pratyaya Coach 的"Persona 提炼与确认包生成"能力。完整的 Persona 工作流由主 agent 编排（见 `agents/pratyaya.md`），本 skill 不编排主流程，只在被调用时执行以下两个独立 Stage：
...
1. Persona 框架 `frameworks/persona-frame.md`：9 基本信息、6 宫格、4 质量鉴别维度、最低结论要求；
2. `references/persona-spec.md`：Persona Key Points 与确认包的固定 section 定义、ID 约定、Canvas 锚点映射。

只提取用户实际讨论的内容。框架之外的方法或术语不自动成为必填项、补问项或放行条件；只有用户明确使用时，才按原话记录。

## 引用层级（重要）

**不引用逐字稿段落**。与 GC / HMW / MVL 蒸馏一致的立场——头脑风暴的逐字稿不具备段落级权威性：同一人在讨论前后可能表达矛盾立场、口语化试探与跑题占据大量文本、真正的事实来自"确认环节达成的共识"而非某段话。引用应基于：

- **Key Points 内的 section**（如"Persona 关键主张 1"）
- **确认包自身的 section**（如"Persona 缺口表 G01"）
...
Stage 2 完成后交给主 agent 触发闸门（`persona-gate`），不直接进入 Canvas 渲染。

## Stage 1：Key Points 抽取

**目标**：在 30 秒内让用户了解"这次讨论了什么、9 基本信息覆盖度如何、6 宫格覆盖度如何、质量维度判定情况、缺什么"。
...
> 轮次：第 X 轮
> 数据源：transcripts/persona-TXX-raw.md
...
- 矛盾 1：...（如声称用户是"高净值"，但描述的场景是"价格敏感"）
- 矛盾 2：...

## 4. 覆盖度初判

对照 Persona 框架（`frameworks/persona-frame.md`），粗略评估覆盖情况：

| 区域 | 字段 | 状态 | 简评 |
|---|---|---|---|
| 基本信息 | name | 已覆盖 / 部分覆盖 / 未涉及 | ... |
| 基本信息 | gender | ... | ... |
| 基本信息 | age | ... | ... |
| 基本信息 | location | ... | ... |
| 基本信息 | education | ... | ... |
...
| 基本信息 | family_status | ... | ... |
| 基本信息 | income | ... | ... |
| 六宫格 | description | 已覆盖 / 部分覆盖 / 未涉及 | ... |
| 六宫格 | goals_needs | ... | ... |
| 六宫格 | behaviors | ... | ... |
| 六宫格 | pain_points | ... | ... |
| 六宫格 | motivation | ... | ... |
| 六宫格 | decision_factors | ... | ... |
| 质量 | evidence_based（有证据） | 已判定 / 未判定 | ... |
| 质量 | concrete（具体） | ... | ... |
| 质量 | pain_in_voice（用户原话） | ... | ... |
| 质量 | representative（代表性） | ... | ... |
...
- 覆盖度初判必须包含全部 9 基本信息 + 6 宫格 + 4 质量维度。
...
| 宫格 | 内容 | 来源引用 |
|---|---|---|
| description（人物描述） | ... | Persona 关键主张 X |
| goals_needs（目标与需求） | ... | ... |
| behaviors（行为） | ... | ... |
| pain_points（痛点） | ... | ... |
| motivation（动机） | ... | ... |
...
| 维度 | 判定 | 依据 |
|---|---|---|
| evidence_based（有证据） | 通过 / 不通过 | ... |
| concrete（具体） | 通过 / 不通过 | ... |
| pain_in_voice（用户原话） | 通过 / 不通过 | ... |
| representative（代表性） | 通过 / 不通过 | ... |
...
| ID | 结论 | 类型 | 共识状态 |
|---|---|---|---|
| PERSONA-C01 | ... | fact / decision / hypothesis / recommendation | 共识 / 待确认 / 争议 |
| PERSONA-C02 | ... | ... | ... |
...
|---|---|---|---|---|---|
| PERSONA-G01 | blocker | open | ... | ... | ... |
| PERSONA-G02 | major | open | ... | ... | ... |
| PERSONA-G03 | minor | open | ... | ... | ... |

> 等级定义：blocker = 使画像核心产出（基本信息或六宫格）无法成立；major = 显著改变画像的代表性或可信度；minor = 不改变核心结论，可后续补齐或明确接受风险。
> 状态定义：
...
- Persona 关键主张 X 中关于 ...
- Persona 覆盖度初判：{区域} / {字段} {状态}
- Persona 缺口表 G{XX}（如已在本表登记）
...
- Gate 报告摘要：{见 `../persona-gate/references/PERSONA-gate.md`；本字段可写评估项 PASS/FAIL 数与最关键 1–2 项摘要}
...
|---|---|---|---|---|
| PERSONA-GATE-0X | PERSONA-{section} | business_risk | low / medium / high | ... |
...
|---|---|---|---|
| 第 1–11 节业务内容（含基本信息、六宫格、质量、结论、缺口、推断、引用）变化 | **是**（vN → vN+1） | **是** | **是**（清空 `gate_recommendation` / `render_authorized` / `confirmation_mode` / `override_audit`） |
...
3. 9 基本信息中 `name` / `job_title` / `industry` 三个关键项任意项完全未讨论时，整个确认包标为 `gaps_open`，不出 `review_ready`。
4. 6 宫格任意区完全未讨论时，确认包状态为 `gaps_open`，不出 `review_ready`。
5. 推断必须独立登记，不写入结论或固定 Canvas section。
6. 模板 section 缺失时标缺口，不能用通用话术补满。
7. 不引用逐字稿段落；引用应基于 Key Points / 确认包内的 section。
8. 质量鉴别四维度判定必须在讨论中有线索，不能完全由 LLM 构造。第 6a 节判定必须基于转写证据。
9. 未完成人工确认前，不视为提炼完成；状态由主 agent 在"确认 vN"后写入。
