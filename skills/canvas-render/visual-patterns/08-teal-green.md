...
| 用途 | Hex | CSS 变量建议 |
|---|---|---|
| 页面背景 | `#f5f8f7` | `--teal-light` |
| 纸张 / Canvas | `#ffffff` | `--paper` |
| 青绿（结构主色） | `#197a56` | `--teal-green` |
| 深绿 | `#0e3e1b` | `--teal-dark` |
| 亮绿 | `#21bf61` | `--teal-bright` |
| 浅绿 | `#e7f4ec` | `--teal-pale` |
| 主文字 | `#1a1a1a` | `--ink` |
| 辅助文字 | `#5c6b65` | `--muted` |
| 边框 | `#d5e0db` | `--line` |
...
`#197a56` 是标题、section、规则线和结论的唯一结构主色（青调深绿，绿中带青、沉稳）；`#0e3e1b` 负责深色层级，`#21bf61` 只用于方向和连接。

## 字体

- 主字体：`Georgia, "Noto Serif SC", "Songti SC", serif`，不加载网络字体。
- 页面 Hero：`clamp(28px, 3.2vw, 44px)`，字重 700，行高 1.2，颜色 `--teal-green`。
...
- **Hero**：深绿 serif 标题、亮绿 eyebrow 和 4px 深绿规则线。
  - **Pan-Mode Invariants（v2.3.5+）**：Hero 永远白纸底（与 §反例 强约束一致），主绿 `#0c8a4f` 仅作 eyebrow / 标题色 / 4px 主绿底线 / 行动摘要 5px 左线 / pale 卡背景 / 矩阵 2×2 单元 3px 主绿左边线；禁止主色整片涂 hero 底色。
...
- **指标**：24px Teal Green 数字；可用单个深绿信号标注需要战略关注的项。
