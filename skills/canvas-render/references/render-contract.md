...
- `#workflow-flow` 的 SVG 几何可由**官方几何展开工具** `skills/canvas-render/scripts/workflow_layout/workflow_layout.py` 确定性生成（语义拓扑见 `canvas-data.workflow`），或由渲染按本 §A1 派生规则静态生成；两条路径产物均须满足 A1.1–A1.5 全部契约，审计断言不变。
