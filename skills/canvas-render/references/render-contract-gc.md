...
数据源是 `modules/GC-v{N}.md`（确认包，Markdown）；LLM 读取后按本章的 section 映射表，把内容映射到 HTML 锚点。映射的字段名沿用确认包 Markdown 的 section 标题。

## A. 黄金圈 Canvas 页面结构

```html
<body data-mode="formal" data-page-type="golden-circle" data-version="1">
  ...
  <main>
    ...
    <section id="cross-layer-alignment">
      <h2>跨层一致性</h2>
      <div id="alignment-why-how">WHY→HOW 推导链（数据源：GC-v{N}.md 第 6a 节）</div>
      <div id="alignment-how-what">HOW→WHAT 推导链（数据源：GC-v{N}.md 第 6a 节）</div>
    </section>
  </main>
...
- `canvas-data.auth` 字段对应 `state.json.golden_circle`。
...
- `canvas-data` 的 `auth` 字段必须与 `state.json.golden_circle` 完全一致。
...
同 MVL：Python 静态审计（`audit_canvas_html.py --type gc`）+ 浏览器视觉验收。两阶段都通过后才把状态改为 `rendered`。
