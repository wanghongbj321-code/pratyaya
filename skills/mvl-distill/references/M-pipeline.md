...
- 分级渲染验收（L1 静态审计 + L2 双视口 DOM 断言必做，L3 截图目检按需）全过 → `rendered`；
...
- HTML 写出 ≠ 渲染完成：分级渲染验收（L1 静态审计 + L2 双视口 DOM 断言必做，L3 截图目检按需；定义见 `skills/canvas-render/SKILL.md`「分级渲染验收」）全过才置 `rendered`。
