...
- 分级渲染验收（L1 静态审计必做；L2/L3 仅用户明确要求时执行；定义见 `skills/canvas-render/SKILL.md`「分级渲染验收」）全过后才置 `rendered`；首次失败保持 `confirmed`；已有本版本成功产物后的失败保持原 `rendered`、output_file 和文件，不回退状态。
